...
Write-Host ""
Write-Host "Stock & POS development stack is starting (db, redis, api, frontend)."
Write-Host "  Frontend (nginx): http://localhost:$FrontendPort"
Write-Host "  API docs:         http://localhost:$ApiPort/docs"
Write-Host ""
Write-Host "For a local-only production run use: .\scripts\stockpos\start-system.ps1"
