...
async def search(
    ...
    if user_has_permission(actor, "stock.view"):
        ...
        rows = (
            await db.execute(
                select(Product.id, Product.name, Product.sku, StockBalance.quantity)
        ...
        for product_id, name, sku, quantity in rows:
            hits.append({
                "id": str(product_id),
                "type": "product",
                "title": name,
                "subtitle": f"{sku or ''} \u00b7 Qty {_qty(quantity)}" if sku else f"Qty {_qty(quantity)}",
